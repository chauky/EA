		...
		<div class="container">
			<div class="properties-listing spacer">
				<form:form modelAttribute="searchDto" role="form"
					action="searchMovie" method="POST">
					<div class="row">
								...
								<c:forEach var="movie" items="${listMovies}">
									...
									<div class="col-lg-4 col-sm-6">
										<div class="properties">
											...
											<p>
												<strong>${movie.getClass().simpleName}</strong>
											</p>
											<h4>${movie.name}</h4>
											<p>${movie.descirption}</p>
											<p>${movie.buildGeneresAsString()}</p>
											<p>
												<em>Director:</em> ${movie.director.name}
											</p>
											<div class="listing-detail">
												<p>
													<img alt="ratting"
														src="<c:url value="/resources/images/star.jpg"/>"
														width="30" height="25"> <strong>rating:</strong> <span
														data-toggle="tooltip" data-placement="bottom"
														data-original-title="Ratting">${movie.rating} </span>
												</p>
											</div>
											<c:if test="${movie.getClass().simpleName == 'TVSeries'}">
												<div class="listing-detail">
													<p>
														<em>No of Seasons :</em> <span data-toggle="tooltip"
															data-placement="bottom" data-original-title="Seasons">${movie.seasons.size()}
														</span>
													</p>
												</div>
											</c:if>
											<a class="btn btn-primary"
												href="<c:url value="/property/propertyList?id=${property.id}"/>">View
												Details</a>
										</div>
									</div>
									<!-- properties -->
